...
export const TOOLS = [
  ...
  },
  {
    ...
    function: {
      ...
      description: 'Delegate sub-task to different model (Hermes 4, Claude, GPT-5). Returns full reply. Default: Hermes 4 405B.',
      parameters: {
        type: 'object',
        properties: {
          task: { type: 'string', description: 'Complete task instruction' },
          model: { type: 'string', description: 'nousresearch/hermes-4-405b (default), anthropic/claude-sonnet-4.6, openai/gpt-5, moonshotai/kimi-k2-thinking' },
        },
        required: ['task'],
      },
    },
  ...
  try {
    switch (call.name) {
      ...
      case "dispatch_subagent": {
        ...
        const model = String(args.model || "nousresearch/hermes-4-405b");
